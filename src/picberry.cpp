...
volatile int pic_clk = DEFAULT_PIC_CLK;
...
volatile inline void delay_us(volatile unsigned int howLong) {
  struct timeval tNow, tNow1, tLong, tEnd;
  tLong.tv_sec = howLong / 1000000;
  tLong.tv_usec = howLong % 1000000;
  while(timercmp(&tNow, &tNow1, !=))
  {
    gettimeofday(&tNow, 0);
    timeradd(&tNow, &tLong, &tEnd);
    gettimeofday(&tNow1, 0);
  }
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "hS:l:g:c:s:f:r:w:ebdR", long_options,
                            &option_index)) != -1) {
    switch (opt) {
    case 0:
      break;
  ...
  if (log) {
    cout << "picberry PIC Programmer v" << VERSION << " - Output to log file."
         << endl;
  ...
  if (flags.debug) {
    cout << "PGC <=> pin " << pic_clk_port << (pic_clk & 0xFF) << endl;
    cout << "PGD <=> pin " << pic_data_port << (pic_data & 0xFF) << endl;
    cout << "MCLR <=> pin " << pic_mclr_port << (pic_mclr & 0xFF) << endl;
  }

  /* Setup gpio pointer for direct register access */
  if (flags.debug)
    cout << "Setting up I/O..." << endl;
  setup_io();
  ...
  if (function == FXN_RESET)
    ...
    // pic = new dspic33f(); default case, nothing to do
      ...
      pic = new pic10f322();
    else if (strcmp(family, "pic16f178x") == 0)
      pic = new pic16f178x();
    ...
    cout << "entering program mode" << endl;

    /* ENTER PROGRAM MODE */
    pic->enter_program_mode();

    if (pic->read_device_id()) { // Read device ID and setup memory
...
/* print the help */
void usage(void) {
  cout << "Usage: picberry [options] \n"
          "\n"
          "   Programming Options \n"
          "\n"
          "       --help,             -h                print help\n"
          "       --server=port,      -S port           server mode, "
          "listening on given port\n"
          "       --log=[file],       -l [file]         redirect the output "
          "to log file(s)\n"
          "       --gpio=PGC,PGD,MCLR -g PGC,PGD,MCLR   GPIO selection in "
          "form [PORT:]NUM (optional)\n"
          "       --family=[family],  -f [family]       PIC family [default: "
          "dspic33f]\n"
          "       --read=[file.hex],  -r [file.hex]     read chip to file "
          "[defaults to ofile.hex]\n"
          "       --write=file.hex,   -w file.hex       bulk erase and write "
          "chip\n"
          "       --erase,            -e                bulk erase chip\n"
          "       --blankcheck,       -b                blank check of the "
          "chip\n"
          "       --regdump,          -d                read configuration "
          "registers\n"
          "       --noverify                            skip memory "
          "verification after writing\n"
          "       --debug                               turn ON debug\n"
          "       --fulldump                            don't detect empty "
          "sections, make complete dump (PIC32)\n"
          "       --program-only                        read/write only "
          "program section (PIC32)\n"
          "       --boot-only                           read/write only boot "
          "section (PIC32)\n"
          "\n"
          "\n"
          "   Runtime Options\n"
...
void server_mode(int port) {
  ...
  memset(&pbserver, 0, sizeof(pbserver));
  pbserver.sin_family = AF_INET;
  pbserver.sin_addr.s_addr = htonl(INADDR_ANY);
  pbserver.sin_port = htons(port);
  ...
  while (1) {
    ...
    cout << "Client connected: " << inet_ntoa(pbclient.sin_addr) << endl;
    /* Receive message */
    if ((received = recv(clientsock, buffer, BUFFSIZE, 0)) < 0)
      cout << "Failed to receive initial bytes from client";
    /* Send bytes and check for more incoming data in loop */
    while (received > 0) {
      if (flags.debug) {
        buffer[received] = '\0';
        cout << "Command received: " << buffer;
      }

      switch (buffer[0]) {
      case SRV_PB_VER:
        cout << "[CMD] Get picberry version" << endl;
      ...
      case SRV_ENTER:
        if (!program_mode) {
          cout << "[CMD] Enter Program Mode" << endl;
          pic->enter_program_mode();
          if (pic->setup_pe())
            program_mode = true;
          else
      ...
      case SRV_EXIT:
        if (program_mode) {
          cout << "[CMD] Exit Program Mode" << endl;
          pic->exit_program_mode();
          program_mode = false;
        }
        break;
      ...
      case SRV_DEV_ID:
        if (program_mode) {
          if (flags.debug)
            cout << "[CMD] Read Device ID" << endl;
          if (pic->read_device_id())
            fprintf(stdout,
                    "{\"DevName\" : \"%s\", \"DevID\" : \"0x%08X\", "
                    "\"DevRev\" : \"0x%08X\"}",
                    pic->name, pic->device_id, pic->device_rev);
          else {
            fprintf(stdout, "NC");
            pic->exit_program_mode();
            program_mode = false;
