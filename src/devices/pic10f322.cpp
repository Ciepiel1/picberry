...
/* set Table Pointer */
void pic10f322::reset_mem_location (void) { send_cmd (COMM_RESET_ADDR, DELAY_TDLY); }
...
bool pic10f322::read_device_id (void) {
    ...
    for (unsigned short i = 0;
         i < sizeof (detailed_subfamily_table) / sizeof (detailed_subfamily_table[0]);
         i++) {
...
void pic10f322::write (char *infile) {
    ...
    for (addr = 0; addr < mem.code_memory_size;
         addr += latch_size) { /* address in WORDS (2 Bytes) */
        ...
        for (i = 0; i < latch_size - 1; i++) { /* write the first 62 bytes */
            if (mem.filled[addr + i]) {
                if (flags.debug)
                    fprintf (stdout,
                             "  Writing 0x%04X to address 0x%06X \n",
                             mem.location[addr + i],
                             (addr + i));
                ...
                if (flags.debug)
                    fprintf (stdout, "  Writing 0x3FFF to address 0x%06X \n", (addr + i));
                send_cmd (COMM_LOAD_FOR_PROG, DELAY_TDLY);
        ...
        if (mem.filled[addr + latch_size - 1]) {
            if (flags.debug)
                fprintf (stdout,
                         "  Writing 0x%04X to address 0x%06X and then start "
                         "programming...\n",
            ...
            if (flags.debug)
                fprintf (stdout,
                         "  Writing 0x3FFF to address 0x%06X and then start "
                         "programming...\n",
                         (addr + latch_size - 1));
            send_cmd (COMM_LOAD_FOR_PROG, DELAY_TDLY);
            write_data (0x3FFF); /* write 0x3FFF in empty locations */
    ...
    if (!flags.noverify) {
        ...
        for (addr = 0; addr < mem.code_memory_size; addr++) {
            ...
            if (flags.debug)
                fprintf (stdout,
                         "addr = 0x%06X:  pic = 0x%04X, file = 0x%04X\n",
                         addr,
                         data,
            ...
            if ((data != mem.location[addr]) & (mem.filled[addr])) {
                fprintf (stderr,
                         "Error at addr = 0x%06X:  pic = 0x%04X, file = "
                         "0x%04X.\nExiting...",
                         addr,
                         data,
                         mem.location[addr]);
        ...
        /* NOTE: It is impossible to program LVP bit when Low-Voltage
         * Programming. We will ignore LVP bit in Configuration Fuse by using
         * 0x3EFF mask.
         */
        uint16_t mask = 0x3FFF;
